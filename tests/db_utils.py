...
async def create_database_async(
    ...
    database = url_item.database
    dialect_name = url_item.get_dialect().name
    dialect_driver = url_item.get_dialect().driver

    if dialect_name == "postgresql":
        url_item = _set_url_database(url_item, database="postgres")
    elif dialect_name == "mssql":
        url_item = _set_url_database(url_item, database="master")
    elif dialect_name == "cockroachdb":
        url_item = _set_url_database(url_item, database="defaultdb")
    elif not dialect_name == "sqlite":
        url_item = _set_url_database(url_item, database=None)
    ...
    if (dialect_name == "mssql" and dialect_driver in {"pymssql", "pyodbc"}) or (
        ...
        engine = create_async_engine(url_item, isolation_level="AUTOCOMMIT")
    else:
        engine = create_async_engine(url_item)
...
async def drop_database_async(url: str) -> None:
    ...
    if dialect_name == "mssql" and dialect_driver in {"pymssql", "pyodbc"}:
        engine = create_async_engine(url_item, connect_args={"autocommit": True})
